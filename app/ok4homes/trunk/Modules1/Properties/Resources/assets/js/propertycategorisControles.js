...
/* ************************************************************************* */  
    ...
    if($('#property_category_list').length){
        $('#property_category_list').DataTable({
        processing: true,
        serverSide: true,  
        ajax: base_url+"/o4k/property_category/AdminPropertyCategoryList",
            columns: [ 
                { data: 'id', name: 'id' },
                { data: 'name', name: 'name' },
                ...
                '<li><a href="'+base_url+'/o4k/property_category/edit/'+full.id+'"><i class=" icon-pen"></i> Edit Property Category</a></li>';
				 if(full.status=="0"){
                            u+= '<li ><a class="change_status"  href="'+base_url+'/o4k/property_category/activate/'+full.id+'"><i class=" icon-eye"></i> Activate </a></li>';
                        }
                  else{
                            u+='<li  ><a class="change_status" href="'+base_url+'/o4k/property_category/deactivate/'+full.id+'"><i class="  icon-eye-blocked"></i> Deactivate </a></li>';
                        } 
                u+='<li><a Onclick="return ConfirmDelete();" class="delete_single" href="'+base_url+'/o4k/property_category/destroy/'+full.id+'"><i class="icon-trash"></i> Delete Property Category</a></li>';
                '</ul></li></ul>';                     
                return u;
                }
...
/*
  * create form 
     ...
     */
     $("#property_category_create").submit(function(e)
    {
       e.preventDefault(); 
...
//        /* ------------------------------------------------------------------ */
        ...
        if(a==1 && b==1 && c==1)
        {
           ...
           $.ajax({
                    type: "POST",
                    url:base_url+"/o4k/property_category/store",
                    dataType: "json",
                    async: false, 
                    data: new FormData($('#property_category_create')[0]),
                    processData: false,
                    contentType: false, 
...
/* ************************************************************************* */  
    ...
    $("#property_category_update").submit(function(e)
    {
        e.preventDefault(); 
...
//        /* ------------------------------------------------------------------ */
        ...
        if(a==1 && b==1 && c==1)
        {
           ...
           $.ajax({
                    type: "POST",
                    url: base_url+"/o4k/property_category/update/"+$("#property_category_update").attr('data-id'), 
                    data: new FormData($('#property_category_update')[0]),
                    dataType: "json",  
                    cache:false,
                    contentType: false,                   
                    processData:false,
