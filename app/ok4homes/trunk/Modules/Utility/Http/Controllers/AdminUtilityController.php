...
class AdminUtilityController extends Controller
{
     ...
     * @return Response
     */
    public function allUtilityList()
    {
    ...
    public function create()
    {
        ...
        return view('utility::admin.create',compact('countries'));
    }
    ...
    { 
        if($request->ajax())
        {
            ...
            if(!empty($languages))
            {  
                $returnHTML = (String) view('utility::admin.sections.dynamic',compact('languages'));
        ...
        $created_country=$request->country;
                ...
                if(!empty($error)){
                    ...
                    if (empty($tableStatus)) {
                        ...
                        $request->session()->flash('msg', "Utility not created successfully.Table not found"); 
                        ...
                        if(empty($utilityId)){
                            $request->session()->flash('val', 0);
                            $request->session()->flash('msg', "Utility not created successfully.Agent module not found");
                            return response()->json(['status'=>false,'csrf' => csrf_token()]);
                        }else{
                            ...
                            try{
                                $user->save();
                                /* Save user module */
                                $user->types()->attach($utilityId->id);
                                ...
                                if( count(Mail::failures()) > 0 ) {
                                    ...
                                    $request->session()->flash('msg', "Utility created successfully.<span style='color:#fe3d3d'>Unable to connect the email </span>"); 
                                    return response()->json(['status'=>true,'csrf' => csrf_token()]);
                                }else{
                                    $request->session()->flash('val', 1);
                                    $request->session()->flash('msg', "Utility created successfully !");
                                    return response()->json(['status'=>true,'url'=>URL('/o4k/utility/'),'csrf' => csrf_token()]);
                                }
                                ...
                                $request->session()->flash('val', 0);
                                $request->session()->flash('msg', "Utility not created successfully.".$ex->getMessage()); 
                                return response()->json(['status'=>false,'csrf' => csrf_token()]);
    ...
    public function edit($id)
    {
        ...
        if($user==null){return redirect('/o4k/404');}
            ...
            $countries=Countries::with(['created_countries'=>function($query) {$query->where('status', 1);}])->get();
            return view('utility::admin.edit',compact('user','countries'));
        }
    }
    ...
    public function getName($countryid,$userId,Request $request){
        ...
        if($request->ajax())
        {
            ...
            if(!empty($nameArray))
            {  
                $returnHTML = (String) view('utility::admin.sections.dynamic_edit',compact('nameArray'));
                return response()->json(['status'=>true, 'html'=>$returnHTML,'csrf' => csrf_token()]);
            }else{return response()->json(['status'=>false, 'message'=>'Oops something went wrong','csrf' => csrf_token()]);}
        }else{return redirect('/404');}
        ...
        $created_country=$request->country;
                ...
                if(!empty($error)){
                   ...
                   $user      = Users::find($id);
                    if($user==null){ 
                        $request->session()->flash('val', 0);
                        $request->session()->flash('msg', "Utility not updated successfully.".$e->getMessage()); 
                        return response()->json(['status'=>false,'csrf' => csrf_token()]);
                    }else{
                            ...
                            try{
                                ...
                                $request->session()->flash('val', 0);
                                $request->session()->flash('msg', "Utility not updated successfully.".$ex->getMessage()); 
                                return response()->json(['status'=>false,'csrf' => csrf_token()]);
    ...
    public function destroy($id)
    {
        ...
        { 
            ...
            { 
                $user->delete();
                Session::flash('val', 1);
                Session::flash('msg', "Utility deleted successfully !");

            } catch (Exception $ex) {
                Session::flash('val', 1);
                Session::flash('msg', $ex->getMessage());
            } 
            return redirect('o4k/utility');
        }
    }
    ...
    public function activate($id)
    {
        ...
        { 
            ...
            { 
                ...
                $user->save();
                Session::flash('val', 1);
                Session::flash('msg', "Utility activated successfully !");
            }catch (Exception $ex) {
                Session::flash('val', 1);
                Session::flash('msg', $ex->getMessage());
            } 
    ...
    public function deactivate($id)
    {
        ...
        { 
            ...
            { 
                ...
                $user->save();
                Session::flash('val', 1);
                Session::flash('msg', "Utility deactivated successfully !");
            }catch (Exception $ex) {
                Session::flash('val', 1);
                Session::flash('msg', $ex->getMessage());
            } 
