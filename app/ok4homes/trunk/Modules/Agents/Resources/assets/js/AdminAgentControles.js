  ...
  utilsScript: base_url+"/public/site/js/plugin/utils.js"  ,
   ...
   //initialize selectbox
    ...
    $("#country_id").change(function() 
    {
        ...
        $.ajax({
            type: 'GET',
            url: base_url+"/o4k/agents/getlanguage/"+countryId, 
            dataType: 'json',
    ...
    if($("#agents_update").length)
    {
        var userId = $( "#agents_update" ).attr("data-id" );  
        var countryId=$( "#country_id" ).val();
        var countryflag=$(this).find(':selected').attr("data-flag" );  
        $("#mnumber").intlTelInput("setCountry", countryflag);
        $.ajax({
            type: 'GET',
            url: base_url+"/o4k/agents/getName/"+countryId+"/"+userId, 
            dataType: 'json',
            success: function (data) {
            if(data.status==true) { $(".langsection").html(data.html); }
            else { $("#error-common").html(data.message);  }
...
/* ************************************************************************* */  
   ...
   if($('#agents_list').length){
        ...
        $('#agents_list').DataTable({
            processing: true,
            serverSide: true,  
            ajax: base_url+"/o4k/agents/AdminAgentsList",
            columns: [ 
                { data: 'id', name: 'id' },
                { data: 'name', name: 'name' },
                ...
                render: function (data, type, full) {  
                    ...
                    '<li><a href="'+base_url+'/o4k/agents/edit/'+full.id+'"><i class=" icon-pen"></i> Edit Agent</a></li>';
                    if(full.status=="0")
                    {
                        u+= '<li ><a class="change_status"  href="'+base_url+'/o4k/agents/activate/'+full.id+'"><i class=" icon-eye"></i> Activate Agent</a></li>';
                    }
                    else
                    {
                        u+='<li  ><a class="change_status" href="'+base_url+'/o4k/agents/deactivate/'+full.id+'"><i class="  icon-eye-blocked"></i> Deactivate Agent</a></li>';
                    } 
                    u+='<li><a Onclick="return ConfirmDelete();" class="delete_single" href="'+base_url+'/o4k/agents/destroy/'+full.id+'"><i class="icon-trash"></i> Delete Agent</a></li>';
                    '</ul></li></ul>';                     
                    return u;
                }
...
*/
    $("#agents_create").submit(function(e)
    {
		
        e.preventDefault();
        var formdata    = new FormData($('#agents_create')[0])
...
/* ************************************************************************* */ 
 ...
 /*
    ...
    * params : Name,Status,Slug  
*/
   $("#agents_update").submit(function(e)
   {
		
       e.preventDefault();
        var formdata    = new FormData($('#agents_update')[0])
        var country     = $("[name='countries']").val().trim();
        var email       = $("[name='email']").val().trim();
        var mobile      = $("[name='mnumber']").intlTelInput("getNumber");
       ...
       if(a===1 && b===1 && c===1 && d===1 && e===1)
        {
               $.ajax({
                    type: "POST",
                    url: base_url+"/o4k/agents/update/"+$("#agents_update").attr('data-id'), 
                    dataType: "json",
                    async: false, 
                    data: formdata,
                    processData: false,
