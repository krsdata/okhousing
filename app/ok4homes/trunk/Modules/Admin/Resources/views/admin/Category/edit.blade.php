...
@section('content') 
    <div class="panel panel-flat">
        ...
        <div class="row">
            <div class="col-md-12">
                <form action="#" id="Category_list_update" autocomplete="off" data-id="{{ $Category->id}}"> 
                    ...
                    <!---Name-->
                        ...
                        <div class="form-group col-md-6" id="titlebox_en">
                            ...
                            <input type="hidden" name="language_en" value="1">
                            <input  id= "title_en" name="title_en" type="text" class="form-control perm_text" placeholder="Enter title" data-lang="en" value="{{$Category->title}}">
                    ...
                    $ids = $Category->types->pluck('id')->toArray();
                    $lang_ids = $Category->types->pluck('language_id')->toArray();
                    $titles = $Category->types->pluck('title')->toArray();
                    $slugs = $Category->types->pluck('slug')->toArray();
                   ...
                   <div class="row">
                     ...
                     <div class="form-group col-md-6" id="CategoryTypebox">
                                ...
                                <select class="bootstrap-select " data-width="100%"  name="CategoryType" id="CategoryType">
                                        ...
                                        <option value="{{$Type->id}}"  @if($Category->master_category_id == $Type->id ) selected @endif >{{$Type->title}}</option>
                                        @endforeach
                                </select>
                                <span  class="help-block"></span> 
                            </div>
                    ...
                    @foreach($languages as $language)
                            ...
                            <input type="hidden" name="ids[]" value="@if(!empty($Category->types[0])){{$ids[$key]}}@else{{$Category->id}}@endif" >
                            <div class="form-group col-md-6"  id="namebox_{{$language->languages->lang_code}}">
                                <label>Name in {{$language->languages->name}}</label>
                                <input id="title_{{$language->languages->lang_code}}" name="title[]" type="text" class="form-control perm_text" placeholder="Enter title" value="@if(!empty($Category->types[0])){{$titles[$key]}}@endif">
                                <input   name="short_name[]" type="hidden" value='{{$language->languages->lang_code}}' class="form-control" value="@if(!empty($Category->types[0])){{$titles[$key]}}@endif">
                                 <span  class="help-block "></span> 
                            </div>
