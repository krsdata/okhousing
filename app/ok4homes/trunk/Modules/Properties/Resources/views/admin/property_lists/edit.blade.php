					...
					<div class="form-group col-md-6" id="statusbox">
							<label>Status</label>
								<select class="bootstrap-select" name="status" data-width="100%">
									...
									<option value="0" {{($property_lists->status==0)?'selected':''}}>Inactive</option>
								</select>
								<span  class="help-block"></span> 
						</div>
							...
							<label>Building Unit</label>
								<select class="bootstrap-select" name="building_unit" data-width="100%" id="building_unit">
									...
									@foreach($buildingunits as $buildingunit)
										<option value="{{$buildingunit->id}}" @if(in_array($buildingunit->id,$selectedbunit) || in_array($buildingunit->parent_id,$selectedbunit)) selected @endif>{{$buildingunit->unit}}</option>
									@endforeach
								</select>
								<span  class="help-block"></span> 
						</div>
							...
							?>
								<select class="bootstrap-select" name="land_unit" data-width="100%" id="land_unit">
									...
									@foreach($landunits as $landunit)
										<option value="{{$landunit->id}}" @if(in_array($landunit->id,$selectedlunit) || in_array($landunit->parent_id,$selectedlunit)) selected @endif>{{$landunit->land_unit}}</option>
									@endforeach
								</select>
								<span  class="help-block"></span> 
						</div>
						...
						<div class="form-group col-md-12" id="amenitybox" style="overflow-y: auto;border: 1px solid rgba(128,128,128,.25);margin-left: 28px;min-height: 200px;width: 972px;">
							...
							@foreach($amineties as $aminety)
								<label class="checkboxa" style="padding-right: 15px;">
								    <input type="checkbox" class="checker border-success text-success-600  CheckboxStyle amenities" name="aminety[]" value="{{$aminety->id}}" @if(in_array($aminety->id,$selectedvalue) || in_array($aminety->parent_id,$selectedvalue)) checked @endif>
									{{$aminety->name}}
								</label>
							@endforeach
							...
							@foreach($neighbourhoods as $key=>$neighbourhood)
								<div class="form-group col-md-2" id="neighbourbox_{{$key+1}}">
									<label class="checkboxa" style="margin-top: 15px!important;">
										...
										<input name="in_km[]" id="km_id_{{$key+1}}"  type="text" class="form-control" placeholder="Enter km" style="width: 123px!important;" @foreach($selectedneigh as $selected)  @if($selected['neighbourhood_id']==$neighbourhood['id']  || $selected['neighbourhood_id']==$neighbourhood['parent_id']) value="{{$selected['kilometer']}}" @endif @endforeach>
									<span  class="help-block"></span> 		 
								</div>
