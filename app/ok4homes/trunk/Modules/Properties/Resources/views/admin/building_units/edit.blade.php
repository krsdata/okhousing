				...
				<div class="row">
					<!---Name-->
						...
						<div class="form-group col-md-6" id="unitbox_en">
							...
							<input  id= "unit_en" name="unit_en" type="text" class="form-control perm_text" placeholder="Enter Unit" data-lang="en" value="{{$building_unit->unit}}">
						...
						<div class="form-group col-md-6" id="slugbox_en">
							<label>Slug</label>
								<input id="slug_en"  type="text" name="slug_en"  readonly class="form-control perm_slug" placeholder="Building Unit Slug"  value="{{$building_unit->slug}}">
								 <span  class="help-block"></span> 
						</div>
				</div>
				@php
					$ids = $building_unit->types->pluck('id')->toArray();
					$lang_ids = $building_unit->types->pluck('language_id')->toArray();
					$units = $building_unit->types->pluck('unit')->toArray();
					$slugs = $building_unit->types->pluck('slug')->toArray();
				@endphp
						@foreach($languages as $language)
							<div class="row">
									...
									<input type="hidden" name="ids[]" value="@if(!empty($building_unit->types[0])){{$ids[$key]}}@endif" >
									<div class="form-group col-md-6"  id="unitbox_{{$language->languages->lang_code}}">
										<label>Name in {{$language->languages->name}}</label>
										<input id="unit_{{$language->languages->lang_code}}" name="unit[]" type="text" class="form-control perm_text" placeholder="Enter Unit" value="@if(!empty($building_unit->types[0])){{$units[$key]}}@endif">
								...
								<div class="form-group col-md-6" id="slugbox_{{$language->languages->lang_code}}">
									<label>Slug</label>
									<input  type="text" id="slug_{{$language->languages->lang_code}}" name="slug[]" readonly class="form-control perm_slug" placeholder="Building Unit Slug"  value="@if(!empty($building_unit->types[0])){{$slugs[$key]}}@endif">
									 <span  class="help-block "></span> 
								</div>
							</div>
						@endforeach
					<div class="row">
						...
						<div class="form-group col-md-6" id="statusbox">
							...
							<select class="bootstrap-select" name="status_en" id="building_unit_status"data-width="100%">
								<option value="1" {{($building_unit->status==1)?'selected':''}} >Active</option>
								<option value="0" {{($building_unit->status==0)?'selected':''}}>Inactive</option>
							</select>
							 <span  class="help-block "></span>
						</div>
