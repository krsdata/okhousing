                       ...
                       <div class="form_wrapper">
                        <div class="input-field input_category input_wrapper ">
                            <select class="appenddiv selectclass" name="searchCategory">
                               ...
                               @endphp
                               @foreach($categorylist as $cat)
                                <option value="{{($cat->parent_id == null)?$cat->id:$cat->parent_id}}">{{$cat->name}}</option>
                               @endforeach
                            </select>
                        </div>

                        <div class="input-field input_wrapper location_input">
                            <input   type="text" class="validate" id="searchLocation_{{ trim($row->id)}}" name="searchLocation"  placeholder="" onkeypress="autocompleteLocation('searchLocation_{{ trim($row->id)}}')" />
                        ...
                        <div class="search_btn input_wrapper">
                            <a href="javascript::void(0);" onclick="searchFun('Search_form_{{ trim($row->id)}}' ,'{{ trim($row->id)}}');">{{trans('countries::home/home.Search')}}</a>
                        </div>
...
<input type="hidden" id="hidden_active_div" value="">
 ...
 <script src="https://maps.googleapis.com/maps/api/js?key=AIzaSyCRXJxQZefgiAbDF2U7Qqv8PNoqBgRiYUc&v=3.exp&sensor=false&libraries=places"></script>
