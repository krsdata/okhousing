...
class FrontPropertyController extends Controller
{
    ...
    public function index()
    {
       ...
       // dd($amenities);
        ...
        return view('website::web.property.add_property', compact('users','property_categories','property_types','countries','building_units','land_units','amenities','neighbourhoods','languages','CategoryType'));
 ...
 $Types_property = implode(',', $request->property_type);
        ...
        }else
        {
            ...
            try{
                ...
                $request->session()->flash('msg', "Property created successfully !  Property is under review.");
...
//This is for which Type select only that category shown.
                  ...
                  $property_types = PropertyType::where(array('status'=>1,'language_id'=>$property_lists->language_id))->get();
                    ...
                    $countries=Countries::with(['created_countries'=>function($query) {$query->where('status', 1);}])->get();
                    $building_units=BuildingUnits::where(array('status'=>1,'language_id'=>$property_lists->language_id))->get();
                    $land_units=LandUnits::where(array('status'=>1,'language_id'=>$property_lists->language_id))->get();
                    $amenities =Amenities::where(array('status'=>1,'language_id'=>$property_lists->language_id))->get();
         ...
         $Types_Count=1;
                ...
                if($Types_Count)
                {
                 ...
                 foreach ($newarray as  $tasrow) { 
                    $resultdata .='<option value="'.$tasrow->id.'">'.$tasrow->name.'</option>';
                 }
