...
{ 
    ...
    public function update_password(Request $request)
    {
        ...
        if($user){
            ...
            $user->save();
            return response()->json(['status'=>1, 'html'=>'Password updated successfully']);
        }
        else
        {
           return response()->json(['status'=>0,'html'=>'something went wrong']);
        }
    ...
    public function UpdateProfile(Request $request)
    {
        ...
        if(Session::has('fcountry')){
            ...
            if($validator->fails()){return response()->json(['status'=>0,'errstep'=>1,'errArr'=>$validator->errors()]);}
                ...
                if($checkExist > 0){
                    return response()->json(['status'=>0,'errstep'=>2,'errArr'=>"Mobile number already taken"]);
                }
                else
                {
