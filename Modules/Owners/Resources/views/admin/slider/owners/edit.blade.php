...
@section('content') 
    <div class="panel panel-flat">
        ...
        <div class="panel-heading">
            <h5 class="panel-title">Update Slider Owners List<a class="heading-elements-toggle"> <i class="icon-more"></i> </a></h5>
        </div>
		<div class="panel-body">
		<!--- Form -->
            <form action="#" id="slider_Owners_list_update" autocomplete="off" data-id="{{$Owners->id}}">
				<div class="row">
						<!---Owners Name-->
							...
							<select class="bootstrap-select " data-width="100%"  name="slider_element_id" id="Owners_id">
                                <option value="" >Select Owner</option>
									@foreach($OwnerList as $Owner)
										<option value="{{$Owner['id']}}" @if($Owner['id'] == $Owners->slider_element_id )) selected @endif >{{$Owner['name']}}</option>
									@endforeach
							</select>
						...
						<div class="form-group col-md-4" id="pagebox">
							...
							<select class="bootstrap-select " data-width="100%"  name="page[]" id="showpage_id" multiple="true">
                                <?php $Pages = explode(",", $Owners->page_type); ?>
									...
									<option value="Utility"  @if(in_array('Utility',$Pages)) selected @endif>Owners</option>
