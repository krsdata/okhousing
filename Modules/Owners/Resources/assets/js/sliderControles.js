...
/* ************************************************************************* */  
    ...
    if($('#property_list').length){
        ...
        processing: true,
        serverSide: true,  
        ajax: base_url+"/o4k/sliderowners/sliderowners",
            columns: [ 
                { data: 'id', name: 'id' },
                { data: 'name', name: 'name'},
                ...
                '<li><a href="'+base_url+'/o4k/sliderowners/edit/'+full.id+'"><i class=" icon-pen"></i> Edit Slider Owner </a></li>';
                  u+='<li><a Onclick="return ConfirmDelete();" class="delete_single" href="'+base_url+'/o4k/sliderowners/destroy/'+full.id+'"><i class="icon-trash"></i> Delete Slider Owner</a></li>';
                '</ul></li></ul>';                     
                return u;
                }
...
/*
  * create form 
     ...
     */
    $("#slider_Owners_list_create").submit(function(e)
    {

        e.preventDefault(); 

        var Owners_id=$("#Owners_id").val().trim();
        var page_id=$("#showpage_id").val();
        var a=b=0;
         //proerty name
        if(Owners_id.length > 0){
           a=1;  
            $( "#Ownersbox" ).removeClass( "has-error" );
            $("#Ownersbox .help-block").html(' ');
        }
        else{
            a=0; 
            $( "#Ownersbox" ).addClass( "has-error" ); 
            $("#Ownersbox .help-block").html('<label id="default_select-error" class="validation-error-label" for="default_select">This field is required.</label>');
        }
...
//        /* ------------------------------------------------------------------ */
        ...
        if(a==1 && b==1)
        {
             ...
             $.ajax({
                    type: "POST",
                    url: base_url+"/o4k/sliderowners/store", 
                    data: new FormData($('#slider_Owners_list_create')[0]),
                    dataType: "json",  
                    cache:false,
                    contentType: false,                   
                    processData:false,
                    ...
                    error: function (request, textStatus, errorThrown) {
                        ...
                        {
                           $( "#Ownersbox" ).addClass( "has-error" );
                           $("#Ownersbox .help-block").html("<div class='mad'>"+request.responseJSON.errors.slider_element_id[0]+"</div>");   
                        }      
    ...
    $("#slider_Owners_list_update").submit(function(e)
    {
        e.preventDefault(); 
...
//        /* ------------------------------------------------------------------ */
        ...
        if(a==1 && b==1)
        {
             ...
             $.ajax({
                    type: "POST",
                    url: base_url+"/o4k/sliderowners/update/"+$("#slider_Owners_list_update").attr('data-id'), 
                    data: new FormData($('#slider_Owners_list_update')[0]),
                    dataType: "json",  
                    cache:false,
                    contentType: false,                   
                    processData:false,
