     ...
     $("#country_id").change(function() {
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                $("#bunitbox").html(data.buHTML);
                $("#lunitbox").html(data.luHTML);
...
/* ************************************************************************* */  
    ...
    if($('#property_list').length){
        ...
        ordering: false ,
        bDestroy: true,
        ajax: base_url+"/o4k/property_list/UserpropertyList/"+FilterCountry,
            columns: [ 
                { data: 'id', name: 'id',sortable: true  },
                { data: 'name', name: 'name' ,sortable: true },
                { data: 'country', name: 'country' ,sortable: true },
                { data: 'property_category.name', name: 'property_category.name','orderable': true, },
                {
  ...
  * create form 
     ...
     $("#property_list_create").submit(function(e)
    {
       ...
       // alert(user_name);
        ...
        var bulding_area_show = $("#property_category").find(':selected').data('bulding_area_show');
        ...
        var landarea_show = $("#property_category").find(':selected').data('landarea_show');
        ...
        var bedroom_show = $("#property_category").find(':selected').data('bedroom_show');
        ...
        var bathroom_show = $("#property_category").find(':selected').data('bathroom_show');
        ...
        var a=b=c=d=e=f=g=h=i=j=k=l=m=n=p=q=r=s=t=0;
     ...
     if(property_type.length > 0){
       ...
       /* if(featured_image > 0){
        ...
        var p =1;
        var fileName = $("#property_image").val();
        if(fileName !=='')
           {
                var bytes = $("#property_image")[0].files[0].size;
...
//        /* ------------------------------------------------------------------ */
        ...
        if(p==1 && a==1  && c==1 && d==1 && e==1 && f==1 && g==1 && h==1 && i==1 && j==1 && k==1 && l==1 && n==1  && q==1)
        {
...
function initMap() {
    ...
    if(pac_input == '')
    {
       var country_name = $(e).find("option:selected").text();
    }
   ...
   //alert(country_name);
    ...
    var geocoder =  new google.maps.Geocoder();
      geocoder.geocode( { 'address': country_name = $(e).find("option:selected").text()}, function(results, status) {
...
function initMap1() {
   ...
   var map = new google.maps.Map(document.getElementById('map'), {
    ...
    var geocoder =  new google.maps.Geocoder();
      geocoder.geocode( { 'address': country_name = $("#pac-input").val()}, function(results, status) {
        if (status == google.maps.GeocoderStatus.OK) {
             map.setCenter(results[0].geometry.location);
             var latitude = results[0].geometry.location.lat();
            var longitude = results[0].geometry.location.lng();
...
$( "#property_type" ).on( "change", function() {
  ...
  if(curentSelect!='')
  {
     $.ajax({
                    type: "POST",
                    url: base_url+"/o4k/property_list/selectcategory", 
                    data: {id:curentSelect},
                    success: function(response){
                   console.log(response);

                    $("#property_category").html(response);
                   // $('#property_category').selectpicker();
                  //  alert("hi");
                  // $("#property_typechanged").append(response);
