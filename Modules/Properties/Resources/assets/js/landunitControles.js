...
/* ************************************************************************* */  
    ...
    if($('#land_unit_list').length){
        ...
        processing: true,
        serverSide: true,  
        ajax: base_url+"/o4k/land_unit/AdminLandUnitList",
            columns: [ 
                { data: 'id', name: 'id' },
                ...
                '<li><a href="'+base_url+'/o4k/land_unit/edit/'+full.id+'"><i class=" icon-pen"></i> Edit Land Unit</a></li>';
				 if(full.status=="0"){
                            u+= '<li ><a class="change_status"  href="'+base_url+'/o4k/land_unit/activate/'+full.id+'"><i class=" icon-eye"></i> Activate </a></li>';
                        }
                  else{
                            u+='<li  ><a class="change_status" href="'+base_url+'/o4k/land_unit/deactivate/'+full.id+'"><i class="  icon-eye-blocked"></i> Deactivate </a></li>';
                        } 
                u+='<li><a Onclick="return ConfirmDelete();" class="delete_single" href="'+base_url+'/o4k/land_unit/destroy/'+full.id+'"><i class="icon-trash"></i> Delete Land Unit</a></li>';
                '</ul></li></ul>';                     
                return u;
                }
...
//        /* ------------------------------------------------------------------ */
        ...
        if(a==1 && b==1 && c==1 )
        {
           ...
           $.ajax({
                    type: "POST",
                    url:base_url+"/o4k/land_unit/store",
                    dataType: "json",
                    async: false, 
...
//        /* ------------------------------------------------------------------ */
        ...
        if(a==1 && b==1 && c==1)
        {
           ...
           $.ajax({
                    type: "POST",
                    url: base_url+"/o4k/land_unit/update/"+$("#land_unit_update").attr('data-id'), 
  ...
  //fetch language and append based on selected country
     $("#country_id").change(function() {
        ...
        $.ajax({
            type: 'GET',
            url: base_url+"/o4k/land_unit/getlanguage/"+countryId, 
            dataType: 'json',
  ...
  //fetch language and append based on selected country
     $("#country_id_edit").change(function() {
        ...
        $.ajax({
            type: 'GET',
            url: base_url+"/o4k/land_unit/getlanguage_edit/"+countryId+"/"+parent_id, 
            dataType: 'json',
            success: function (data) {
                $("#pac-input").val('');
                if(data.status==true) { $(".lang_section").html(data.html); }
