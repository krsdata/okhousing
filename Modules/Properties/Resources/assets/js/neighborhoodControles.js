...
/* ************************************************************************* */ 
	...
	if($('#neighborhood_list').length){
        $('#neighborhood_list').DataTable({
        processing: true,
        serverSide: true,  
        ajax: base_url+"/o4k/neighborhood/AdminNeighborhoodList",
            columns: [ 
                { data: 'id', name: 'id' },
                { data: 'name', name: 'name' },
                ...
                '<li><a href="'+base_url+'/o4k/neighborhood/edit/'+full.id+'"><i class=" icon-pen"></i> Edit Neighborhood</a></li>';
                if(full.status=="0"){
                            u+= '<li ><a class="change_status"  href="'+base_url+'/o4k/neighborhood/activate/'+full.id+'"><i class=" icon-eye"></i> Activate </a></li>';
                        }
                  else{
                            u+='<li  ><a class="change_status" href="'+base_url+'/o4k/neighborhood/deactivate/'+full.id+'"><i class="  icon-eye-blocked"></i> Deactivate </a></li>';
                        } 
                u+='<li><a Onclick="return ConfirmDelete();" class="delete_single" href="'+base_url+'/o4k/neighborhood/destroy/'+full.id+'"><i class="icon-trash"></i> Delete </a></li>';
                '</ul></li></ul>';                     
                return u;
                }
...
/*
    ...
    $("#neighborhood_create").submit(function(e)
    {
       e.preventDefault(); 
         
        var Status         =   $("[name='status_en']").val().trim();
        var icon           =   $("#neighborhood_image").get(0).files.length
        var a=0;
        var b=0;
        var c=0; 
		var d=0;
...
//        /* ------------------------------------------------------------------ */
        ...
        if(a==1 && b==1 && c==1 )
        {
               ...
               $.ajax({
                    type: "POST",
                    url:base_url+"/o4k/neighborhood/store",
                    dataType: "json",
                    async: false, 
                    data: new FormData($('#neighborhood_create')[0]),
                    processData: false,
                    contentType: false, 
...
/* ************************************************************************* */  
 ...
 /*
     ...
     */
      ...
      $("#neighborhood_update").submit(function(e)
    {
        e.preventDefault(); 
        var Status         =   $("[name='status_en']").val().trim();
...
//        /* ------------------------------------------------------------------ */
        ...
        if(a==1 && b==1 && c==1 )
        {
           ...
           $.ajax({
                    type: "POST",
                    url: base_url+"/o4k/neighborhood/update/"+$("#neighborhood_update").attr('data-id'), 
                    data: new FormData($('#neighborhood_update')[0]),
                    dataType: "json",  
                    cache:false,
                    contentType: false,                   
                    processData:false,
