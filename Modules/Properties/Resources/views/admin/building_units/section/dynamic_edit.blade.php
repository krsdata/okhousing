  @if(isset($languages))
                            @foreach ($languages as $key => $value)
                               ...
                               $BuildingUnitsEach = Modules\Properties\Entities\BuildingUnits::where('language_id' ,$value['languages']['id'])->where('country_id',$value['created_country_id'])->whereNull('deleted_at')->where(function ($query) use ($parent_id) {
