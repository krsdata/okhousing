...
*/
    ...
    {   
        ...
        $country_role = ['countries' => array_combine($countries, $roles)];
        ...
        {           
            ...
            $admin->role_details = json_encode($country_role);
...
*/
    ...
    public function storebackgroundimg(Request $request)
    {
     ...
     public function Adevertise()
    {
        return view('admin::admin.Adevertise');
    }

    public function AdevertiseList()
    {
         return Datatables::of(Advertise::orderBy('id', 'DESC')->get())->make(true);
    }
     ...
     * @return Response
     */
    public function SearchRadius()
    {
        
        $SearchRadius = SearchRadius::first();
        if($SearchRadius==null){return redirect('/o4k/404');}
        else{
            return view('admin::admin.SearchRadius', compact('SearchRadius'));
        }
     ...
     * @return Response
     */
    public function UpdateSearchRadius(Request $request)
    {
        
        $SearchRadius = SearchRadius::find($request->id);
        if($SearchRadius==null){ 
        $request->session()->flash('val', 0);
        $request->session()->flash('msg', "Search Radius not updated successfully.".$e->getMessage());
        return redirect('/o4k/SearchRadius'); 
        }
        else
        {
            $SearchRadius->radius = $request->radius; 
             try{
                $SearchRadius->save();
                $request->session()->flash('val', 1);
                $request->session()->flash('msg', "Search Radius updated successfully !");
                return redirect('/o4k/SearchRadius');
