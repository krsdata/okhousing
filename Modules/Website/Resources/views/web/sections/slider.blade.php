...
for($row= 2; $row<= 3; $row++)
{
    for($loop = 1; $loop <= 6; $loop++) {
        ...
        if(@$SliderList[$id])
        {
            $Slide = $SliderList[$id];
        ?>
    <a href="{{url('search/property/'.$Slide['id'].'/'.str_slug($Slide['title']))}}"
...
@endif
@if(Session::get('guest_id') && !Auth::guard('front_user')->user())
@php 

$uid = Session::get('guest_id');
$FvrtPropertyCount = Modules\Website\Entities\Wishlist::where('user_id', $uid)->count();
@endphp
        <a href="{{($FvrtPropertyCount > 0 )?URL('/search?type=1&Tab=Favorites'):'#'}}" class="{{($FvrtPropertyCount > 0 )?'fvractive':''}}" data-title="{{trans('countries::home/home.favorites')}}({{$FvrtPropertyCount}})"></a>
