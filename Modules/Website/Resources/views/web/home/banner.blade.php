        ...
        <div class="search_wrapper hide">
            ...
            <ul class="nav nav-tabs search_nav">
                ...
                @foreach( $categorylistType as $row)
                    ...
                    <li id="{{ $row->id }}" class=" @if($count==1) active @endif masterid" > <a data-toggle="tab" href="#TabID_{{trim($row->id)}}"> 
                            @if($row->title =='Sell')
                            Buy
                            @else
                              {{ $row->title }} 
                    ...
                    <div class="col-xs-6">
                        <select class="propertiType" id="MobileFilter" >
                            ...
                            @php if($count==1) $selectedType = ($row->parent_id == null)?$row->id:$row->parent_id; @endphp
                                 <option value="TabID_{{trim($row->id)}}" data-id="{{$row->id}}" @if($count==1) selected @endif >@if(trim($row->title)=='Sell') Buy @else {{trim($row->title)}} @endif
                    ...
                    <div class="col-xs-6" id="ProjectCategorySelect">
                        <select class="ProjectCategory" name="MsearchCategory">
                            <option value="" disabled selected>{{trans('countries::home/home.Category')}}</option>
                               @php 
                               $categorylist=Modules\Properties\Entities\PropertyCategory::where('master_category_id','like', '%' . $selectedType.'%')->where('status','1')->where('language_id',$langId)->orderby('id','ASC')->get();
                               @endphp
                       ...
                       <div class="form_wrapper">
                        <div class="input-field input_category input_wrapper ">
                            <select class="appenddiv selectclass" name="searchCategory">
                                <option value="" disabled selected >{{trans('countries::home/home.Category')}}</option>
                               @php 
                               $categorylist=Modules\Properties\Entities\PropertyCategory::where('master_category_id','like', '%' . $cat_type .'%')->where('status','1')->where('language_id',$langId)->orderby('id','ASC')->get();
                               @endphp
                               @foreach($categorylist as $cat)
