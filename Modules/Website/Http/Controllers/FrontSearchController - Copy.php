...
class FrontSearchController extends Controller
{
    ...
    public function ShowProperty($id)
    {
        ...
        else
        {
	       ...
	       return response()->json(['status'=>true,'html'=>$returnHTML] , JSON_UNESCAPED_UNICODE);
  ...
  public function searchFunByuniqueID($id)
    {
			...
			$SearchList = PropertyList::with('property_created_amenities','property_created_neighbourhoods')->where('uid',$id)->where('status',1)->where('country_id', "=",$countryId)->where('language_id', "=",$langId)->get();
