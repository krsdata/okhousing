  ...
  @if(isset($nameArray))
    @foreach ($nameArray as $key => $value)
       ...
       <div class="form-group col-md-6 about{{$key}} chkabout" >
            ...
            <textarea style="resize: none" rows="3" cols="5" data-lagabout="{{$key}}" data-aboutlan="{{$value['country_lang_id']}}" data-about_langcode="{{$value['lang_code']}}" class="form-control" placeholder="About Us in {{$value['lang_name']}}"  name="about[]" >{{$value['user_about']}}</textarea>
            <span  class="help-block"></span> 
        </div>
