...
use Modules\Projects\Entities\SliderList;
use Modules\Projects\Entities\SliderPageOrder;
use Modules\Admin\Entities\FeaturedProjects;
use Modules\Website\Entities\Wishlist;
use Modules\Projects\Entities\PropertyNeighbourhoods;
...
class PropertyListController extends Controller
{
    ...
    {
        dd('1');
        return view('projects::admin.property_lists.index',compact('id'));
    }
    ...
    public function create()
    {
     ...
     //   $property_types = PropertyType::where(array('status'=>1,'language_id'=>1))->get();
        ...
        return view('projects::admin.property_lists.create', compact('users','property_categories','countries','buildingunits','landunits','amineties','neighbourhoods','CategoryType'));
    ...
    public function getlanguage($countryid,Request $request)
    {
		if($request->ajax())
        {
            ...
            {  
                ...
                $buHTML = (String) view('projects::admin.property_lists.section.dynamicbu',compact('BuildingUnits'));
                $luHTML = (String) view('projects::admin.property_lists.section.dynamiclu',compact('landunits'));

                
                $returnHTML = (String) view('projects::admin.property_lists.section.dynamic',compact('languages'));
       ...
       // echo"<pre>"; print_r($property_lists);die;
        ...
        else
        {
           ...
           $CategoryType=CategoryType::select('title','id','parent_id')->where('status','1')->where('language_id',1)->orderby('title','ASC')->get();
            ...
            return view('projects::admin.property_lists.edit', compact('users','property_categories','property_types','countries','buildingunits','landunits','amineties','neighbourhoods','property_lists','langugeDetails','galleryimages','CategoryType','property_categoriesedit'));
        }
        ...
        { 
            ...
            { 
                ...
                if( $propertylists)
                {
                 ...
                 FeaturedProjects::where('property_id',$id)->get()->each->delete();
                 Wishlist::where('property_id' ,$id)->get()->each->delete();
                    ...
                    $contactName='';
                    Mail::send('projects::admin.notification',  $data, function($message) use ($toEmail)
                    {
                        $message->to($toEmail);
