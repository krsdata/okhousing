...
/* ************************************************************************* */  
    ...
    if($('#property_type_list').length){
        ...
        processing: true,
        serverSide: true,  
        ajax: base_url+"/o4k/project_types/AdminpropertytypeList",
            columns: [ 
                { data: 'id', name: 'id' },
                { data: 'name', name: 'name' },
                ...
                '<li><a href="'+base_url+'/o4k/project_types/edit/'+full.id+'"><i class=" icon-pen"></i> Edit Property Types</a></li>';
				 if(full.status=="0"){
                            u+= '<li ><a class="change_status"  href="'+base_url+'/o4k/project_types/activate/'+full.id+'"><i class=" icon-eye"></i> Activate </a></li>';
                        }
                  else{
                            u+='<li  ><a class="change_status" href="'+base_url+'/o4k/project_types/deactivate/'+full.id+'"><i class="  icon-eye-blocked"></i> Deactivate </a></li>';
                        } 
                u+='<li><a Onclick="return ConfirmDelete();" class="delete_single" href="'+base_url+'/o4k/project_types/destroy/'+full.id+'"><i class="icon-trash"></i> Delete Property Types</a></li>';
                '</ul></li></ul>';                     
                return u;
                }
  ...
  * create form 
     ...
     $("#property_type_create").submit(function(e)
    {
       e.preventDefault(); 
        ...
        var CategoryType         =   $("[name='CategoryType']").val().trim();
        var a=0;
        var b=0;
        var c=0; 
        var d=0;
        ...
        if(CategoryType.length > 0 ){
           d=1;  
            ...
            $("#CategoryTypebox .help-block").html('<label id="default_select-error" class="validation-error-label" for="default_select">Invalid status.</label>');
        }
...
//        /* ------------------------------------------------------------------ */
        ...
        if(a==1 && b==1 && c==1 && d==1)
        {
           ...
           $.ajax({
                    type: "POST",
                    url:base_url+"/o4k/project_types/store",
                    dataType: "json",
                    async: false, 
...
//        /* ------------------------------------------------------------------ */
        ...
        if(a==1 && b==1 && c==1 && d==1)
        {
           ...
           $.ajax({
                    type: "POST",
                    url: base_url+"/o4k/project_types/update/"+$("#property_type_update").attr('data-id'), 
