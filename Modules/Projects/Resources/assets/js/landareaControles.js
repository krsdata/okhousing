...
/* ************************************************************************* */  
    ...
    if($('#land_area_list').length){
        $('#land_area_list').DataTable({
        processing: true,
        serverSide: true,  
        ajax: base_url+"/o4k/projects/area/AdminLandAreaList",
            columns: [ 
                { data: 'id', name: 'id' },
                { data: 'slug', name: 'slug' },
                ...
                '<li><a href="'+base_url+'/o4k/projects/area/edit/'+full.id+'"><i class=" icon-pen"></i> Edit Land Area</a></li>';
				 if(full.status=="0"){
                            u+= '<li ><a class="change_status"  href="'+base_url+'/o4k/projects/area/activate/'+full.id+'"><i class=" icon-eye"></i> Activate </a></li>';
                        }
                  else{
                            u+='<li  ><a class="change_status" href="'+base_url+'/o4k/projects/area/deactivate/'+full.id+'"><i class="  icon-eye-blocked"></i> Deactivate </a></li>';
                        } 
                u+='<li><a Onclick="return ConfirmDelete();" class="delete_single" href="'+base_url+'/o4k/projects/area/destroy/'+full.id+'"><i class="icon-trash"></i> Delete Land Area</a></li>';
                '</ul></li></ul>';                     
                return u;
                }
...
/*
  * create form 
     ...
     */
     $("#land_area_create").submit(function(e)
    {
       e.preventDefault(); 
        ...
        $( ".perm_text" ).each(function( index ) {
            ...
            {
                $( "#areabox_"+id[1] ).removeClass( "has-error" );
                $("#areabox_"+id[1]+" .help-block").html(' '); 
            }else
            {
                VName.push(id[1]);
                $( "#areabox_"+id[1] ).addClass( "has-error" ); 
                $("#areabox_"+id[1]+" .help-block").html('<label id="default_select-error" class="validation-error-label" for="default_select">This field is required.</label>');   
            ...
            //slug validation
                 ...
                 {
                    var gnsg= generate_slug($("#area_"+id[1]).val());
                    if(gnsg != valueSlug)
                    { 
                        VSlug.push(id[1]);
...
//        /* ------------------------------------------------------------------ */
        ...
        if(a==1 && b==1 && c==1 )
        {
           ...
           $.ajax({
                    type: "POST",
                    url:base_url+"/o4k/projects/area/store",
                    dataType: "json",
                    async: false, 
                    data: new FormData($('#land_area_create')[0]),
                    processData: false,
                    contentType: false, 
                    ...
                    error: function (request, textStatus, errorThrown) {
                            ...
                            if(obj.hasOwnProperty("area_en") )
                            {
                               $( "#areabox_en" ).addClass( "has-error" );
                               $("#areabox_en .help-block").html("<div class='mad'>"+request.responseJSON.errors.area_en[0]+"</div>");   
                            }         
...
/* ************************************************************************* */ 
    ...
    $('body').on('keyup', '.perm_text', function() {
        var Oid=$(this).attr('id');
        var id=Oid.split('_');  
        $("#slug_"+id[1]).val(generate_slug($("#area_"+id[1]).val()));
    });
...
/* ************************************************************************* */  
    ...
    $("#land_area_update").submit(function(e)
    {
        e.preventDefault(); 
...
//        /* ------------------------------------------------------------------ */
        ...
        if(a==1 && b==1 && c==1)
        {
           ...
           $.ajax({
                    type: "POST",
                    url: base_url+"/o4k/projects/area/update/"+$("#land_area_update").attr('data-id'), 
                    data: new FormData($('#land_area_update')[0]),
                    dataType: "json",  
                    cache:false,
                    contentType: false,                   
                    processData:false,
  ...
  //fetch language and append based on selected country
     $("#country_id").change(function() {
        ...
        $.ajax({
            type: 'GET',
            url: base_url+"/o4k/projects/area/getlanguage/"+countryId, 
            dataType: 'json',
  ...
  //fetch language and append based on selected country
     $("#country_id_edit").change(function() {
        ...
        $.ajax({
            type: 'GET',
            url: base_url+"/o4k/projects/area/getlanguage_edit/"+countryId+"/"+parent_id, 
            dataType: 'json',
            success: function (data) {
                $("#pac-input").val('');
                if(data.status==true) { $(".lang_section").html(data.html); }
